  /*
* Drag-nBot es una creación de Dragón
* Drag-nBot no tiene ningun fin de lucro
...
function addMetadata(packname, author) {	
	if (!packname) packname = 'Drag-nBot'; if (!author) author = 'Dragón';	
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hola mi loco!!! @${num.split('@')[0]} ¿Todo bien NEFASTO?\n\n『Bienvenido A Este Infierno Llamado *${mdata.subject}*』\n\nNo Me Da Gusto Conocerte🧐\n\n_Recuerda Leer Las Reglas O Te Doy Ban En El Culo_\n\n*Disfruta Y Pasame Pack Si Eres Morra :D*`
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo usa Drag-nBot',
		...
		if (!isUser) return reply(mess.only.daftarB)
			...
			case 'tagall':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 *TODOS* 〙✪══\n╠➥'+teks+'╚═〘 _*by Dragón*_ 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
